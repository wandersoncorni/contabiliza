...
export function loadTable(url, table) {
...
export function loadFilters(colSearch = 1, colProfile = 2, colStatus = 3) {
...
export function getButtons(uid, url = '/api/v1/users'){
    return $('<div />').append([
                    ...
                    $('<div />', { class: 'dropdown-menu dashboard-dropdown dropdown-menu-start mt-2 py-1' }).append(
                        $('<a />', { class: 'dropdown-item text-info', href: `${url}/${uid}`, 'data-action': 'show' }).append(
                            $('<i />', { class: 'heroicon heroicon-eye me-2' }),
                            'Ver'
                        ),
                        $('<a />', { class: 'dropdown-item text-primary', href: `${url}/${uid}`, 'data-action': 'edit' }).append(
                            $('<i />', { class: 'heroicon heroicon-pencil-square me-2' }),
                            'Editar'
                        ),
                        $('<a />', { class: 'dropdown-item text-danger', href: `${url}/${uid}`, 'data-action': 'delete' }).append(
...
export function getActive(data){
    return data.active ? $('<span />', { class: 'text-success' }).text('Ativo').prop('outerHTML') : $('<span />', { class: 'text-danger' }).text('Inativo').prop('outerHTML')
}
